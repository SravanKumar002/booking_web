...
let display_hostel_detail_view = document.getElementById(
  "display_hostel_detail_view"
);
...
function createAndAppendCard(item) {
  let { name, address, location, id, image, description, price } = item;
  let { rating } = item.facilities;
  ...
  card_click.onclick = () => {
    ...
    let detail_hostel_image = (document.getElementById(
      "detail_hostel_image"
    ).src = image);
    let detail_name_of_hostel = (document.getElementById(
      "detail_name_of_hostel"
    ).textContent = name);
    let detail_address_of_hostel = (document.getElementById(
      "detail_address_of_hostel"
    ).textContent = location);
    let detail_description_of_hostel = (document.getElementById(
      "detail_description_of_hostel"
    ).textContent = description);
    let detail_prise_of_hostel = (document.getElementById(
      "detail_prise_of_hostel"
    ).textContent = price + "/-");

    let detail_rating = (document.getElementById("detail_rating").innerHTML =
      h_rating);
  };
}
...
function createCards() {
  ...
  if (count === 0) {
    ...
    nodataimg.src =
      "https://res.cloudinary.com/dldcgj0mx/image/upload/v1666773543/nodata1_z41sip.jpg";
...
function getfilterSearch() {
  ...
  let count = 0;
  for (let item of hostelsData) {
    let { location, wifi, hotwater, rating, bedsharing } = item.facilities;

    if (
      locationFilteEle.value == location &&
      wifiFilteEle.checked == wifi &&
      waterFilteEle.checked == hotwater &&
      getRating == rating &&
      bedsharing.includes(bedShare)
    ) {
      createAndAppendCard(item);
      count = count + 1;
    }
...
function getfilterSearchLg() {
  ...
  for (let item of hostelsData) {
    ...
    if (
      locationFilteEleLg.value == location &&
      wifiFilteEleLg.checked == wifi &&
      waterFilteEleLg.checked == hotwater &&
      getRatingLg == rating &&
      bedsharing.includes(bedShareLg)
    ) {
      createAndAppendCard(item);
...
let url = "https://api.npoint.io/5ff6a53ada33bc6e3e3a";
let options = {
  method: "GET",
};
...
user_registration.onclick = (event) => {
  ...
  if (user_registred_data.mail && user_registred_data.password) {
    registration_message.innerHTML =
      "Registration was successful; please login now.	&#128512";
...
redirect_hostelview.onclick = () => {
  ...
  let entered_user_password = document.getElementById(
    "entered_user_password"
  ).value;
  console.log(user_registred_data.mail, user_registred_data.password);
  console.log(entered_user_mail, entered_user_password);

  if (
    user_registred_data.mail == entered_user_mail &&
    user_registred_data.password == entered_user_password &&
    entered_user_mail != "" &&
    entered_user_password != ""
  ) {
...
let chat_bot_message_container = document.getElementById(
  "chat_bot_message_container"
);
...
let quit_message_to_detail = (document.getElementById(
  "quit_message_to_detail"
).onclick = () => {
  chat_bot_message_container.classList.toggle("make_page_none");
  page_2.classList.toggle("make_page_none");
});
